...
| M6 | KL Divergence | `y_true`, **`prob_class_*`** | `y_pred` | — | **prob 필수** |
...
3. **M6 KL Divergence는 prob_class_* 필수** — 하드 레이블로는 계산 불가
...
| M18 | Distribution Diff (ML) | `y_true`, `y_pred` *또는* `prob_label_*`+threshold | — | threshold_per_label | 4213 6.5.5 |
...
| `y_true`만 | M23만 (분포 분석만 가능) |

### 예시 (Multiclass)

| 매핑된 컬럼 | 계산 가능한 지표 |
|------------|---------------|
| `y_true`, `y_pred` | M1~M5, M11~M14, M21, M22, M23 |
| `y_true`, `prob_class_*` (argmax로 y_pred 계산) | 위 + M6 |
...
- "검증 실패" 시 구체적으로 어떤 지표가 영향받는지 표시
  - 예: "prob_class_* 합이 1이 아닙니다 → M6 KL Divergence가 부정확할 수 있습니다"
