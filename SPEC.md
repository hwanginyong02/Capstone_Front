...
> **`prob_class_*` / `prob_label_*` 는 현재 받지 않습니다.**
> multiclass·multilabel 지표 중 확률을 읽는 것이 하나도 없어(§2·§3 표 참조) 매핑해도 값을
> 주지 못하는 반면, 매핑되면 해당 컬럼의 결측이 평가 표본을 깎고 범위 이탈은 평가를
> 중단시킵니다. 그래서 매핑 화면 선택지와 LLM 자동 매핑 대상에서 모두 제외했고, 확률 컬럼은
> `ignore` 로 처리됩니다. 아래 두 가지 중 하나가 생기면 다시 도입합니다.
> - 확률로 예측을 파생하는 경로(multiclass argmax / multilabel threshold) — **미구현**
> - 확률이 있어야만 계산되는 순위 기반 지표(AUROC, mAP 등) — **미도입**
...
| ~~`threshold`~~ | float [0,1] | **미구현** — 아래 참조 | — |
| ~~`threshold_per_label`~~ | float [0,1] | **미구현** — 아래 참조 | — |

> **`threshold` / `threshold_per_label` 은 전 계층 미구현입니다.**
> 확률로부터 예측 라벨을 파생하는 경로(binary threshold, multiclass argmax,
> multilabel threshold)가 프론트·API 계약·백엔드 어디에도 없습니다. 따라서 하드 예측
> 없이 확률만 제출하는 경로는 현재 지원되지 않습니다(binary 에서 `score` 만으로
> 계산되는 M9·M10·M19 는 임계값이 필요 없어 예외).
> 구현 시 주의: argmax·threshold 로 만든 예측은 **모델의 실제 출력이 아니라 파생값**이므로,
> 성적서에 파생 사실과 사용한 임계값을 반드시 기재해야 합니다.
...
|----|--------|-----------|-----------|---------|------|
| M1 | Accuracy | `y_true`, `y_pred` *또는* `score`+threshold | — | threshold (score만 있을 때) | 4213 6.3.3 |
| M2 | Precision | `y_true`, `y_pred` *또는* `score`+threshold | — | **positive_class**, threshold | 4213 3.2.9 |
| M3 | Recall | `y_true`, `y_pred` *또는* `score`+threshold | — | **positive_class**, threshold | 4213 3.2.10 |
| M4 | F1 Score | `y_true`, `y_pred` *또는* `score`+threshold | — | **positive_class**, threshold | 4213 3.2.8 |
| M5 | Fβ Score | `y_true`, `y_pred` *또는* `score`+threshold | — | **positive_class**, threshold, **beta** | 4213 6.2.6 |
| M6 | KL Divergence | `y_true`, `y_pred` | — | — | 4213 6.3.5 (카운트 기반) |
| M7 | Specificity | `y_true`, `y_pred` *또는* `score`+threshold | — | **positive_class**, threshold | 4213 3.2.11 |
| M8 | FPR | `y_true`, `y_pred` *또는* `score`+threshold | — | **positive_class**, threshold | 4213 3.2.12 |
...
| M20 | MCC | `y_true`, `y_pred` *또는* `score`+threshold | — | threshold | — |
| M21 | Confusion Matrix | `y_true`, `y_pred` *또는* `score`+threshold | — | threshold | 4213 6.3.2 |
| M22 | Class별 Metric | `y_true`, `y_pred` *또는* `score`+threshold | `score` | **positive_class**, threshold | — |
...
| `y_true`, `score` (+ threshold UI) | 위 + M9, M10, M19 |
...
| `y_true`, `prob_class_*` | 확률 컬럼만으로 추가되는 지표 없음 (범위 검증에만 사용) |
...
| `y_true`, `prob_label_*` (+ threshold UI) | 위 지표 전체 |
